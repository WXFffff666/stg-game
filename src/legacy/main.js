...
(function() {
  ...
  function checkDailyCompleted() {
    try {
      var today = new Date().toISOString().slice(0, 10);
      return localStorage.getItem('DailyRun_' + today) === '1';
  ...
  function markDailyCompleted() {
    try {
      var today = new Date().toISOString().slice(0, 10);
      localStorage.setItem('DailyRun_' + today, '1');
  ...
  function _getSavedLoadout(characterId) {
    try {
      var raw = localStorage.getItem('stg_loadout_' + characterId);
  ...
  function _saveLoadout(characterId, weaponIds) {
    try {
      localStorage.setItem('stg_loadout_' + characterId, JSON.stringify(weaponIds));
    } catch (e) {}
  }
  ...
  function endGame() {
    ...
    if (window.SeasonPassManager && typeof window.SeasonPassManager.onRunComplete === 'function') {
      window.SeasonPassManager.onRunComplete(runStats);
    }
